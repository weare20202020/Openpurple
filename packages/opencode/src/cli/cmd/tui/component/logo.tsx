...
export function Logo(props: { shape?: LogoShape; ink?: RGBA; idle?: boolean; leftInk?: RGBA[]; rightInk?: RGBA[]; rightRowInk?: RGBA[] } = {}) {
        ...
        {(line, index) => (
          <box flexDirection="row" gap={1}>
            <box flexDirection="row">
              {renderLine(line, index(), props.ink ?? theme.textMuted, !!props.ink, 0, frame(), dusk(), idleState(), props.leftInk ? (col) => props.leftInk![Math.min(col, props.leftInk!.length - 1)] : undefined)}
            </box>
            <box flexDirection="row">
              {renderLine(
                ctx.shape.right[index()],
                index(),
                props.rightRowInk?.[index()] ?? props.ink ?? (props.rightInk ? props.rightInk[0]! : theme.text),
                true,
                ctx.LEFT + GAP,
                frame(),
                dusk(),
                idleState(),
                props.rightInk ? (col) => props.rightInk![Math.min(col, props.rightInk!.length - 1)] : undefined,
              )}
            </box>
          </box>
        )}
