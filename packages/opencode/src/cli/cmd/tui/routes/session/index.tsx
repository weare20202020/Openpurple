...
function isFromMessage(parts: Part[]): boolean {
  return parts.some((p) => {
    if (p.type !== "text") return false
    const t = p as TextPart
    return !t.synthetic && (t.text.startsWith("A2A [From ") || t.text.startsWith("⚙send_message"))
  })
}

function stripA2AContent(parts: Part[]): Part[] {
  return parts.map((p) => {
    if (p.type !== "text") return p
    const t = p as TextPart
    if (!t.synthetic && t.text.startsWith("A2A [From ")) {
      const colon = t.text.indexOf("]: ")
      if (colon !== -1) return { ...t, text: t.text.slice(0, colon + 1) }
    }
    if (!t.synthetic && t.text.startsWith("⚙send_message")) {
      const colon = t.text.indexOf(", message=")
      if (colon !== -1) return { ...t, text: t.text.slice(0, colon) + "]" }
    }
    return p
  })
}
                ...
                <For each={messages()}>
                  {(message, index) => (
                    <Switch>
                      ...
                      <Match when={message.role === "user" && isFromMessage(sync.data.part[message.id] ?? [])}>
                        <box paddingLeft={3} marginTop={1}>
                          <text fg={theme.textMuted}>⚙ {stripA2AContent(sync.data.part[message.id] ?? [])
