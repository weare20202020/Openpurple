...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("OpenPurple")
      return
    }
  ...
  createEffect(() => {
    ...
    // Show banner first; do not auto-navigate to any session
    // User enters via /agents or typing a message
    if (sync.status === "complete" && !bannerShown) {
      bannerShown = true
      // Stay on home/banner — no auto-navigation
      if (args.continue) {
        const match = sync.data.session.toSorted((a, b) => b.time.updated - a.time.updated).find((x) => x.parentID === undefined)
        if (match) {
          continued = true
          local.agent.set(match.agent ?? "A")
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to OpenPurple v${result.data.version}. Please restart the application.`,
    )
